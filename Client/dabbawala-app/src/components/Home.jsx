...
function Home() {
  ...
  const [visibleDabbas,setVisibleDabbas] = useState(10)
  const [visibleLocations, setVisibleLocations] = useState(10)

  const meals = Array.from({ length: 4}, (_, i) => `Meal ${i + 1}`);

  const dabbas = Array.from({length: 4},(_,i)=>`Dabba ${i+1}`)

  const locations = Array.from ({length:2},(_,i)=>`Location ${i+1}`)
    ...
    <>
      ...
      <div className='categories'>  
        ...
        <div className='choose_dabba'>
            {dabbas.slice(0, visibleDabbas).map((dabba, index) => (
              <div className="dabba" key={index}>{dabba}</div>
            ))}
        </div>
